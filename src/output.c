...
void WritePlanetFileAccretionSolids (int TimeStep, int n, boolean big) {
  FILE *output;
  char name[256];
  ...
  fprintf (output, "%d\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\t%#.18g\n",
	   TimeStep,
	   Xplanet,
	   Yplanet,
	   Zplanet,
...
int timestep;
{
  ...
  for (k = 0; k < sys->nb; k++) {
    ...
    sys->sigmarms[k] = GetfromPlanetFile (timestep, 9, k);
    sys->eccrms[k] = GetfromPlanetFile (timestep, 10, k);
    sys->incrms[k] = GetfromPlanetFile (timestep, 11, k);
    sys->masscore[k] = GetfromPlanetFile (timestep, 12, k);
    sys->rcore[k] = GetfromPlanetFile (timestep, 13, k);
    sys->accsolids[k] = GetfromPlanetFile (timestep, 14, k);
    sys->massenv[k] = GetfromPlanetFile (timestep, 15, k);
    sys->masscrit[k] = GetfromPlanetFile (timestep, 16, k);
    sys->accgas[k] = GetfromPlanetFile (timestep, 17, k);
    sys->rcap[k] = GetfromPlanetFile (timestep, 18, k);
  }
}
...
void WritePlanetSystemFile (int t, boolean big) {
  ...
  for (i = 0; i < n; i++) {
    ...
    SigmarmsVirtual = Sys->sigmarms[i];
    EccrmsVirtual = Sys->eccrms[i];
    IncrmsVirtual = Sys->incrms[i];
    McoreVirtual = Sys->masscore[i];
    RcoreVirtual = Sys->rcore[i];
    AccretionsolidsVirtual = Sys->accsolids[i];
    MenvVirtual = Sys->massenv[i];
    McritVirtual = Sys->masscrit[i];
    AccretiongasVirtual = Sys->accgas[i];
    RcapVirtual = Sys->rcap[i];
    //    WriteTorqueAndWork(t, i);
#ifndef COREACCRETION
    WritePlanetFile (t, i, big);
#endif
#ifdef COREACCRETION
    WritePlanetFileAccretionSolids (t, i, big);
#endif
  }
}
...
#ifdef COREACCRETION
  if (WRITESIGMAM)
    __WriteField(Sigmam, TimeStep);
  if (WRITEECC)
    __WriteField(Ecc, TimeStep);
  if (WRITEINC)
    __WriteField(Inc, TimeStep);
#endif
