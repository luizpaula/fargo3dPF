...
void AlgoGas () {
  ...
  if (Resistivity_Profiles_Filled == NO) {
...
#ifdef COREACCRETION
      FARGO_SAFE(Computerhogas());
      FARGO_SAFE(Computenumol());
      FARGO_SAFE(Computereymol());
   #ifndef EQUILIBRIUM
      FARGO_SAFE(Computegasdragecc());
      FARGO_SAFE(Computegravecc());
      FARGO_SAFE(Updateecc(dt));
      FARGO_SAFE(Computegasdraginc());
      FARGO_SAFE(Computegravinc());
      FARGO_SAFE(Updateinc(dt));
      for (kp = 0; kp < Nbpl; kp++) FARGO_SAFE(Computeaccretionsolids(dt,kp));
    #else
      for (kp = 0; kp < Nbpl; kp++) FARGO_SAFE(Computeaccretionsolidsequilibrium(dt,kp));
    #endif
    ...
    //for (kp = 0; kp < Nbpl; kp++) Denshom(kp); //nova funçao

    #ifdef GASACCRETION
       for (kp = 0; kp < Nbpl; kp++)
           {
           if (masscrit[kp] < masscore[kp]) Accretiongas(dt,kp);
           }
    #endif
#else
    #ifdef GASACCRETION
       for (kp = 0; kp < Nbpl; kp++) Accretiongas(dt,kp);
    #endif
#endif
