  ...
  for (k = begin_k; k<end_k; k++) {
    for (j = begin_j; j<end_j; j++) {
      for (i = begin_i; i<end_i; i++) {
          ...
          r = sqrt(xpl[0]*xpl[0] + ypl[0]*ypl[0] + zpl[0]*zpl[0]);

          ecc[l] =  1.7*(pow(m,1.0/15.0)*pow(mpl[0],1.0/3.0)*pow(RHOM,2.0/15.0))*pow(B,-1.0/5.0)*pow(CD,-1.0/5.0)*pow(rhogas,-1.0/5.0)*pow(MSTAR,-1.0/3.0)*pow(r,-1.0/5.0);
          inc[l] = ecc[l]/2.0;
