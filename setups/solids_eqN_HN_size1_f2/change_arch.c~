...
void ChangeArch() {
  ...
  Computerhogas = Computerhogas_cpu;
  Computenumol = Computenumol_cpu;
  Computegasdragecc = Computegasdragecc_cpu;
  Computegravecc = Computegravecc_cpu;
  Computegasdraginc = Computegasdraginc_cpu;
  Computegravinc = Computegravinc_cpu;
  Computereymol = Computereymol_cpu;
  Updateinc = Updateinc_cpu;
  Updateecc = Updateecc_cpu;
  Computeaccretionsolids = Computeaccretionsolids_cpu;
  Computeaccretionsolidsequilibrium = Computeaccretionsolidsequilibrium_cpu;
  Accretiongas = Accretiongas_cpu;
  ...
  while (fgets(s, MAXLINELENGTH-1, func_arch) != NULL) {
	...
	if(strval[0] == 'g'){
      ...
      if (strcmp(name, "computerhogas") == 0) {
	if(strval[0] == 'g'){
	  Computerhogas = Computerhogas_gpu;
	  printf("Computerhogas hook runs on the GPU\n");
	}
      }

      if (strcmp(name, "computenumol") == 0) {
	if(strval[0] == 'g'){
	  Computenumol = Computenumol_gpu;
	  printf("Computenumol hook runs on the GPU\n");
	}
      }

      if (strcmp(name, "computereymol") == 0) {
	if(strval[0] == 'g'){
	  Computereymol = Computereymol_gpu;
	  printf("Computereymol hook runs on the GPU\n");
	}
      }

      if (strcmp(name, "computegasdragecc") == 0) {
	if(strval[0] == 'g'){
	  Computegasdragecc = Computegasdragecc_gpu;
	  printf("Computegasdragecc hook runs on the GPU\n");
	}
      }

      if (strcmp(name, "computegravecc") == 0) {
	if(strval[0] == 'g'){
	  Computegravecc = Computegravecc_gpu;
	  printf("Computegravecc hook runs on the GPU\n");
	}
      }

      if (strcmp(name, "computegasdraginc") == 0) {
	if(strval[0] == 'g'){
	  Computegasdraginc = Computegasdraginc_gpu;
	  printf("Computegasdraginc hook runs on the GPU\n");
	}
      }

      if (strcmp(name, "computegravinc") == 0) {
	if(strval[0] == 'g'){
	  Computegravinc = Computegravinc_gpu;
	  printf("Computegravinc hook runs on the GPU\n");
	}
      }

      if (strcmp(name, "updateecc") == 0) {
	if(strval[0] == 'g'){
	  Updateecc = Updateecc_gpu;
	  printf("Updateecc hook runs on the GPU\n");
	}
      }

      if (strcmp(name, "updateinc") == 0) {
	if(strval[0] == 'g'){
	  Updateinc = Updateinc_gpu;
	  printf("Updateinc hook runs on the GPU\n");
	}
      }

      if (strcmp(name, "computeaccretionsolids") == 0) {
	if(strval[0] == 'g'){
	  Computeaccretionsolids = Computeaccretionsolids_gpu;
	  printf("Computeaccretionsolids hook runs on the GPU\n");
	}
      }

      if (strcmp(name, "computeaccretionsolidsequilibrium") == 0) {
	if(strval[0] == 'g'){
	  Computeaccretionsolidsequilibrium = Computeaccretionsolidsequilibrium_gpu;
	  printf("Computeaccretionsolidsequilibrium hook runs on the GPU\n");
	}
      }

      if (strcmp(name, "accretiongas") == 0) {
	if(strval[0] == 'g'){
	  Accretiongas = Accretiongas_gpu;
	  printf("Accretiongas hook runs on the GPU\n");
	}
      }
