...
//<#>
...
#endif
#ifdef ADIABATIC
       vs = sqrt(gamma*(gamma-1.0)*cs[l]/rhogas[l]); 
#endif
#ifdef POLYTROPIC
       vs = sqrt(cs[l]*gamma*pow(rhogas[l],gamma-1.0));
#endif	
       ...
       if((reymol[l] >= 20.0) && (RM >= lambda))
         ...
         c0 = -(1.0/(2.0*tdrag))*inc[l];
         c1 = nfactor*nfactor;
         c2 = (1.0/PI)*epsilon*epsilon*ecc[l]*ecc[l];
         c3 = (4.0/PI)*inc[l]*inc[l];
         c4 = c1 + c2 + c3;
         c5 = c0*sqrt(c1+c2+c3);
         didt[l] = c5;
         }
       ...
       if((reymol[l] < 20.0) && (RM >= lambda))
         {
         didt[l] = -(3.0/8.0)*(lambda/(RHOM*RM*RM));
         didt[l] *= vs*rhogas[l];
         didt[l] *= inc[l];
         }
